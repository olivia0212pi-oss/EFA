...
**M2 − M1 配对 bootstrap（按题目配对，2000 次重采样，见下方"重要限定"）**：省 token 比例差值 +2.68 个百分点，95% CI [+0.30%, +5.22%]，不包含 0；正确率差值 0，CI [-3.23%, +3.23%]。

**风险目标放宽到 10%/15% 时**，M2 在错误率和省 token 上开始同时优于 M0、M1（但这个"同一目标"下的对比同样受下方"重要限定"第 2 条约束）。

**更谨慎的表述（用户复核后修正）**：不能说"已经证实时序动态有真实增益"——只能说**在当前这 93 道偏简单的题目上，观察到一个有希望的正向信号，需要用没参与过特征设计的新数据确认**。原因：

1. 这组时序特征（`confidence_delta`/斜率/骤降等）是看过这 93 题的初步结果后设计出来的。分组交叉验证能防止同一题的训练/测试数据泄漏，但**不能替代真正独立的新数据验证**——特征本身的设计过程已经"看过"这批数据。
2. "target-matched sweep"是同一个校准**目标**，不是同一个**实测**停错率——M0/M1/M2 在同一目标下实际达到的停错率并不相同（如上表 10.13% vs 11.43% vs 11.27%），脚本输出已经把这一点标注清楚，不能当成严格意义上的同风险对比。
3. M1/M2 准确率点估计相同（差值 0），但 bootstrap 区间是 [-3.23%, +3.23%]——只能说"没观察到下降"，不能说"证明了不变"。
4. 之前"扣除探测开销后"的净节省数字**低估了真实开销**：旧代码只统计了置信度计算用到的答案 span（`trial_answer_logprobs` 长度），未闭合的探测直接记成 0 开销，闭合后模型继续生成的复查文字也没算进去。已在 `early_exit/build_checkpoints.py` 里修复（新增 `probe_generated_tokens`——vLLM 每次探测调用实际生成的 token 数，`probe_answer_span_tokens`——其中用于置信度计算的部分，`schema_version` 升到 4），`day46_temporal_features.py` 已优先读取新字段，但**这次跑的 v3 数据是修复前生成的，仍是旧的低估口径**，新字段只会在后续新采集的确认集里生效。

此外，配对 bootstrap 只重采样了单次嵌套 CV 的样本外结果，**没有包含重新拟合/重新校准的方差**，是真实不确定性的下界，不是完整区间。
...
### 下一步：扩到约 300 题作为独立确认集，再做 Day 7

用户判断：不直接扩到 Day 7 跨 benchmark，先扩样本到总计约 300 题（当前 93 题作为开发集，新增约 200 题作为独立确认集，冻结协议后不再依据新结果调整特征/超参），验证 M2 相对 M1 的增益在新题上是否稳定存在，再去 AIME 做跨 benchmark 泛化测试。冻结的协议规格见 `plan.md`。
